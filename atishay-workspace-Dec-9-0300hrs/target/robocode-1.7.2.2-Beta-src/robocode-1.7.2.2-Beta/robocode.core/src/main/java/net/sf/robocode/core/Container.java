...
public final class Container extends ContainerBase {
	...
	static {
		...
		for (String path : classPath.split(File.pathSeparator)) {
			...
			System.out.println("Container.java : calling loadFromPath() for "+path);
			loadFromPath(path);
		}
	...
	private static void loadFromPath(String path) {
		try {
			...
			if (pathf.isDirectory()) {
				System.out.println("Inside Contaner.java loadFromPath() -> path: " + path);
				
				String name = getModuleName(path);

				System.out.println("Inside Contaner.java loadFromPath() -> module_name: " +
						name);
				...
				final int i = test.lastIndexOf("robocode.jar");
					...
					if (i > 0) {
						...
						System.out.println("Container.java : loading jars from "+dir.toString());
						loadJars(dir);
					} else {
						System.out.println("Container.java : loading modules from -- "+path);
						String name = getModuleName(path);
	
						
						if (name != null) {
							System.out.println("Container.java : loading module "+name);
							loadModule(name, engineLoader);
						}
				}
			}
	...
	private static boolean loadModule(String module, ClassLoader loader) {
		try {
			...
			{
				System.out.println("Container.java : loading module starting with CTF");
				modClass = loader.loadClass("CTF.CaptureTheFlagRules");
			}
			else
			{
				System.out.println("Container.java : calling loader.loadClass with module"+module);
					modClass = loader.loadClass(module + ".Module");
			}

			modClass.newInstance();
			Logger.logMessage("Loaded njnjnj" + module);
			known.add(module);
			return true;
	...
	private static String getModuleName(String path) {
		...
		System.out.println("Inside Container.java : getModuleName() -> path :" + path);
		if (test.endsWith("alpha.jar"))
		{
			System.out.println("Container.java : found alpha.jar");
	...
	public static URL[] findJars(String allowed) {
		...
		System.out.println("Inside Container.java : findJars() -> classpath: " + classPath);
		System.out.println("Inside Container.java : findJars() -> Searching for string : " + allowed);

		for (String path : classPath.split(File.pathSeparator)) 
		{
			String test = path.toLowerCase();

			System.out.println("Inside Container.java : findJars() -> test="+test);
			if (test.contains(allowed)) {
				System.out.println("Inside Container.java : findJars() -> checking for jars in " + test);
				if (!test.contains("robocode.jar") && !test.contains("robocode.api")
						) {
					System.out.println("Inside Container.java : findJars() -> jar found at " + path);
					urls.add(path);
				}
			}
		}
